 ...
 * against has the club's real roster in it. Every fixture here is keyed on a
 * `test-sweep-` prefix and every assertion counts only those rows, so a real
 * member being demoted by this suite would be a test failure rather than a
 * silent write. The one query it issues is roster-wide by nature; the
 * `beforeEach` below proves the guard by putting a hand-made member alongside.
 *
...
/** October: fall is running and its trial fortnight is long over. */
