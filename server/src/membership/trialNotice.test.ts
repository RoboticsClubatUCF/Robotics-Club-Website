 ...
 * classes on 24 August, so the twenty-one-day trial closes on 14 September. The
 * fifteenth is one day past that and inside the notice window.
 */
const JUST_AFTER_TRIAL = new Date('2035-09-15T12:00:00')
...
/**
 * Take everybody who is not this file's fixture out of the sweep's way.
 *
 * The sweep reads **fifty candidates a tick** — deliberately, so one pass
 * cannot sit in a loop against Discord's rate limit — and it does not order
 * them. Pinning the clock to 2035 stops this suite writing a row that would
 * suppress a real notice, but it does nothing about who is *considered*: every
 * account in the database was created long before a 2035 deadline and has dues
 * that do not cover 2035, so all of them match. With a seeded database of
 * nineteen that was invisible. With the club's real roster imported it is six
 * hundred, and the fixture is simply not in the batch — every assertion here
 * failed on a sweep that had worked perfectly, fifty rows away.
 *
 * Claiming a 2035 notice for everybody else is the sweep's own mechanism for
 * "already handled", so it leaves exactly one candidate. `clearRows` deletes
 * the year wholesale afterwards, which is why this costs nothing outside the
 * suite.
 */
async function parkEverybodyElse(): Promise<void> {
  const others = await prisma.user.findMany({
    where: { email: { not: EMAIL } },
    select: { id: true },
  })

  await prisma.trialNotice.createMany({
    data: others.map((u) => ({
      userId: u.id,
      termYear: TERM_YEAR,
      termSeason: 'FALL' as const,
      failure: 'parked by trialNotice.test.ts',
    })),
    skipDuplicates: true,
  })
}
