...
export function looksLikeDocument(name: string, bytes: Uint8Array): boolean {
  ...
  if (lower.endsWith('.pdf')) {
    return ascii(bytes, 32).trimStart().startsWith('%PDF-')
  }
  ...
  if (lower.endsWith('.docx')) {
    ...
    const nameLength = new DataView(bytes.buffer, bytes.byteOffset + 26, 2).getUint16(
      0,
      true,
    )
    return ascii(bytes.slice(30), nameLength) === '[Content_Types].xml'
  }
