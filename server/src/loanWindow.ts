 ...
 *
 * Two routers need this and they must agree: `routes/equipment.ts` refuses an
 * ask that runs past the item's cap, and `routes/officer.ts` uses the same
 ...
 * `routes/equipment.ts` do already turn the second into a 400, but only
 ...
 *
 * The browser is guarded separately, in `web/src/lib/borrowing.ts` — there the
