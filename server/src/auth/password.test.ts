...
/**
 * The hashing primitives, on their own. No database, no Hono, no Discord —
 * these are four pure functions and the suite is worth having only because one
 * of them silently returns `false` for anything it cannot parse, which is
 * exactly what a broken import looks like from the outside.
 *
 * The bcrypt half exists because the club's previous site hashed with bcrypt
 * and its members were imported carrying those hashes. What is under test is
 * that a real legacy hash opens, a wrong password against one does not, and
 * `hashPassword` never produces one.
 */

/**
 * Published bcrypt test vectors, from the OpenBSD reference suite by way of
 * jBCrypt — **not** generated by `bcryptjs`.
 *
 * That is the whole point of hard-coding them. A vector this file produced
 * itself would only prove `bcryptjs` agrees with `bcryptjs`; these were
 * produced by a different implementation years ago, so matching them is
 * evidence the library reads what the old site wrote.
 */
