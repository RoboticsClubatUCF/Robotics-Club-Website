...
const envSchema = z.object({
   ...
   *
   * Shorter than the signup one, and one value for both because they are the
   * same risk: a signup link creates an account nobody had, while either of
   * these takes over an account somebody already has. Nobody sits on one of
   * these for two hours — they ask for it because they are trying to get in
   * right now.
   */
   ...
   *
   * Independently optional like the role ids above, and unset means the site
   * still tracks whether the lab is open — the landing page and the dashboard
   * are the record — with nothing pushed to Discord. That is the supported
   * state, not a broken one, and the API says which it is at startup.
   *
   * The bot needs three permissions in this channel and they are different
   * permissions: **Send Messages** and **Read Message History** for the sign
   * itself and **Manage Channels** for the name. Only one of them failing is
   * the awkward case — the message says OPEN while the channel still reads
   * `lab-status-🔴` — so each is logged by name when Discord refuses.
   */
   ...
   *
   * **A seed, and it does not outrank the row.** It cannot: opening the lab
   * posts a *new* message and deletes the one before it, so the id changes
   * every evening — a setting that won would end up pointing at a message this
   * server had itself deleted, and every push would spend a 404 finding that
   * out. The `lab_status` row is what tracks the live one.
   *
   * What it is for is the first push against a channel that already has a sign
   * in it: point this at that message and it is adopted rather than joined by a
   * second one. After that it is never read again, and clearing it is safe.
   *
   * It has to be a message **the bot itself posted**. Discord lets a bot edit
   * only its own messages, so an id belonging to a person is a 403 on every
   * attempt, for ever — logged by name, because it never starts working.
   */
   ...
   *
   * **Almost certainly not needed.** A bot is told about a button press in
   * exactly two ways: down a WebSocket it already holds open, or by an HTTP
   * POST to a public URL registered on the application. The club runs on the
   * first — `src/discord/discordGateway.ts` — which needs no public address, no
   * endpoint URL and no key, because the connection is authenticated once by
   * the bot token.
   *
   * This is for the second, and only becomes relevant once the API is on a real
   * domain with an "Interactions Endpoint URL" registered against it: Discord
   * signs each POST with the application's Ed25519 key and this is the half
   * that checks the signature. Without it `/api/discord/interactions` refuses
   * every delivery, which is the only safe thing an endpoint that cannot check
   * a signature can do — it is unauthenticated, and pressing a button opens a
   * real room.
   *
   * **Setting this does not turn buttons on and leaving it unset does not turn
   * them off.** What decides that is `buttonsLive`: a connected gateway, or an
   * endpoint URL with a key that matches it.
   *
   * 64 hex characters. Not the client secret and not the bot token; those are
   * the two things people paste in here by mistake, and both are the wrong
   * length, which is why the shape is checked rather than trusted.
   *
   * The shape is all that can be checked. *Another* application's public key is
   * also 64 hex characters and imports perfectly, and the only symptom is an
   * endpoint that refuses every delivery — which from inside Discord looks
   ...
   * `originGuard` in `src/routes/account/auth.ts` is what covers that, and it is why
   ...
   *
   * The club's number, and it is a grace period rather than a lead time —
   * the opposite direction from the loan reminder above. A task marked done
   * five minutes late should never have been chased, so the sweep waits for
   * the deadline to be properly past before it says anything.
   *
   * The tick is ten minutes, so what actually lands is 30 to 40 minutes after
   * the deadline. That slack is not worth engineering away: nothing about a
   * task goes wrong in the extra ten minutes, and a timer that fired to the
   * second would be a second scheduler to keep alive.
   */
   ...
   * `<base>/<year>/<season>` — see `src/membership/semester.ts`, which falls back to
  ...
  /**
   * Ceilings for the three kinds of file the site stores, in megabytes. Files
   ...
   *
   * Documents get the middle number because a design review with photographs
   * in it is genuinely larger than a downscaled gallery image and genuinely
   * smaller than a mesh, and unlike both of those nothing shrinks it on the way
   * — a PDF cannot be re-encoded in the browser the way `downscaleImage`
 ...
 * Blank is unset.
 *
 * `FOO=` in a `.env` file arrives as `''`, and an operator who cleared a value
 * meant *off* — not "a value that fails validation", which is what an empty
 * string is against a regex, and which exits the process at boot over a line
 * somebody thought they had disabled.
 *
 ...
 * developer's real `.env` on the next import that re-runs it — so a suite on a
 * configured machine could find itself testing the club's actual bot. An empty
 * string is *present*, so dotenv leaves it alone, and it lands here as absent.
 * `discord.test.ts` says unset that way throughout.
 *
 * Safe across the whole object rather than per field: every variable with a
 * default treats `''` and absent identically already, and no variable here
 * means anything by an empty string.
 */
...
// Nothing is warned about here for the buttons any more, and that is the point:
// whether a press reaches this server is not something `.env` knows. It depends
// on whether the application has an Interactions Endpoint URL registered, which
// is read off Discord at startup — see `confirmInteractionEndpoint`, and the
// line `src/index.ts` logs from it.
if (parsed.data.DISCORD_PUBLIC_KEY && !parsed.data.DISCORD_LAB_CHANNEL_ID) {
  // Still worth one: the endpoint would verify signatures perfectly and there
  // is nothing in any channel carrying a button to press, so it never hears
  // from Discord at all — which reads exactly like a signature that does not
  // verify.
  console.warn(
...
export const env = {
   ...
   * reason: the reset page posts the token back, so a mail scanner following
   * the URL cannot spend it.
   */
  passwordResetUrl: `${parsed.data.SITE_URL}/reset-password`,

  /**
   * Where the email-change confirmation points — the profile page the change
   * was started from, which posts the token back and then says the address has
   * moved. Behind the dashboard's session gate, so somebody opening it on a
   * phone signs in on the way; the gate carries the query string through.
   */
