 ...
 *
 * The same argument as `resetCooldown` in `auth.ts`, and this endpoint is the
 * closer parallel of the two: `writes` is a ten-minute allowance, and an
 * allowance says nothing about *shape* — all five of it can be spent in the
 * same second at five different `@ucf.edu` addresses, which is five students
 * opening their inbox to a confirmation link they never asked for. A script
 * does that faster than the page repaints. Thirty seconds is longer than a mail
 * round trip and shorter than anyone's patience for a link they think went
 * missing, so it costs a real person nothing and costs the loop its whole rate.
 *
 * Its own scope, because a scope holds one row per caller and this is a second
 * window on the same one. Middleware rather than a `consume` in the handler:
 * unlike the contact form's daily budget, spending this on a malformed body is
 * right, because nothing about the shape of a request says whether a person
 * sent it.
 *
 * Deliberately not on `/complete`, which shares `writes` with this. That
 * endpoint mails nobody — it spends a token somebody is holding — and half a
 * minute between attempts at your own password is a punishment for typing.
 */
...
/**
 * And the budget keyed on the *address* rather than on the caller.
 *
 * The mirror of `PER_ADDRESS_MAX` on the password-reset route, for the same
 * reason: this endpoint puts mail in somebody else's inbox, so the thing worth
 * limiting is how often one address can be made to receive it. A per-caller
 * budget alone lets a botnet mail one student a confirmation link all afternoon
 * from a different address every time, and neither of the two windows above
 * would ever close. Three, because nobody legitimately needs a fourth link in
 * ten minutes — the third one is already the "check your spam" attempt.
 */
 ...
 *
 * Exported for `routes/account/account.ts`, which writes the same two columns from the
 * profile page and trips the same two constraints. A second copy of this would
 * drift the next time Prisma moves where the offending column is named — and
 * the symptom of that is a 500 on a case the route already knows how to answer.
 */
...
/** Shared with the profile page's Discord field, which trips the same
    constraint from the other direction — one sentence, one spelling. */
...
signup.post(
  ...
  async (c) => {
    ...
    // Before the lookup, so an address cannot be *asked about* more often than
    // it can be mailed. Keyed on the address itself, which is the only key a
    // caller changing IP on every request cannot get away from.
...
/** What the field is told, which is Discord's four answers plus the one only
    this site can give. */
 ...
 *
 * The whole rule in one place, because two forms now ask it: the signup field
 * below, and the profile page's, which is the same field editing an account
 * that already exists.
 *
 * **`exceptUserId` is what makes it usable from the profile page.** Without it,
 * somebody re-saving the handle they already have is told it is taken — by
 * themselves — which reads as the field being broken. It is a whitelist of
 * exactly one row, so it cannot be used to claim anybody else's.
 *
 * Nothing here writes, and the answer is not trusted at the point of the write:
 * both `complete` and the profile route ask again.
 */
 ...
 *
 * Unauthenticated, so no caller is excused — a signup has no account yet. The
 * profile page has its own copy of this behind `requireAuth`, which passes the
 * caller's id through.
 */
signup.post(
  '/discord-check',
