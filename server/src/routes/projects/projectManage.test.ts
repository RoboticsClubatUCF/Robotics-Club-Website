...
/** Mid-fall 2035: term running, the free opening weeks long over. */
...
describe('the meeting schedule', () => {
  const patch = (body: unknown, cookie = leadCookie) =>
    request('PATCH', `/api/projects/${projectId}`, cookie, body)
  ...
  it('is set by the lead and read back by members', async () => {
    ...
    expect(
      rows.find((row) => row.project.slug === `${PREFIX}rover`)?.project
        .meetingStartTime,
    ).toBe('18:00')
  })

  it('refuses a time that is not a wall-clock time', async () => {
    expect((await patch({ meetingStartTime: 'sixish' })).status).toBe(400)
  })
  ...
  it('refuses to leave a schedule half set', async () => {
    ...
    const response = await patch({ meetingStartTime: null })
    expect(response.status).toBe(400)
  })

  it('lets a lead clear the whole thing', async () => {
    await patch({
      meetingWeekdays: [2, 4],
      meetingStartTime: '18:00',
      meetingEndTime: '22:00',
  ...
  it('refuses meetingsPublic from a lead and takes it from an officer', async () => {
    ...
    const asOfficer = await patch({ meetingsPublic: false }, officerCookie)
    expect(asOfficer.status).toBe(200)
    expect(await asOfficer.json()).toMatchObject({ meetingsPublic: false })
  })
})

describe('creating a project needs a schedule', () => {
  const create = (body: Record<string, unknown>) =>
    request('POST', '/api/officer/projects', officerCookie, {
      slug: `${PREFIX}needs-meeting-${String(Math.random()).slice(2, 8)}`,
      title: 'Needs a meeting',
      summary: 'A thing',
      ...body,
    })

  it('refuses one with no days', async () => {
    expect((await create({ ...MEETING, meetingWeekdays: [] })).status).toBe(400)
  })

  it('refuses one with no times', async () => {
    expect((await create({ meetingWeekdays: [2, 4] })).status).toBe(400)
  })
  ...
  it('takes one that is complete, and starts it on the public calendar', async () => {
    const response = await create({ ...MEETING, meetingLocation: 'ENG2 Lab' })
    ...
    expect(await response.json()).toMatchObject({
      ...
      meetingStartTime: '18:00',
      meetingEndTime: '22:00',
      // Default on, so a new project is findable without anybody remembering
      // to say so.
      meetingsPublic: true,
    })
  })
...
describe('project documents', () => {
  /** `%PDF-` and some filler. The route sniffs the header, not the file. */
  const pdfBytes = (tail = 'one'): Uint8Array<ArrayBuffer> =>
  ...
  function docxBytes(): Uint8Array<ArrayBuffer> {
    ...
    new DataView(bytes.buffer).setUint16(26, name.length, true)
  ...
  function publish(
    ...
    form.append(
      'file',
      new File([fields.bytes ?? pdfBytes()], fields.name ?? 'design-review.pdf', {
        type: 'application/pdf',
      }),
    )
    form.append('title', fields.title ?? 'Design review')
    if (fields.description !== undefined) {
      form.append('description', fields.description)
    }
    form.append('authorUserId', fields.authorUserId ?? leadId)

    return app.request(`/api/projects/${fields.project ?? projectId}/documents`, {
      method: 'POST',
      body: form,
      headers: { cookie },
    })
  ...
  const revise = (
    ...
    bytes: Uint8Array<ArrayBuffer> = pdfBytes('two'),
  ) => {
    const form = new FormData()
  ...
  /** The id and the file id of a freshly published document. */
  async function published(cookie = leadCookie) {
    const response = await publish(cookie)
    expect(response.status).toBe(201)
    return (await response.json()) as { id: string; fileId: string }
  }

  it('publishes a document, credited, with both dates equal', async () => {
    const response = await publish(leadCookie, { description: 'The one to read first.' })
    ...
    expect(document).toMatchObject({
      title: 'Design review',
      description: 'The one to read first.',
      authorName: 'PM Lead',
    ...
    const stored = await prisma.storedFile.findUniqueOrThrow({
      where: { id: document['fileId'] as string },
    })
    expect(stored.kind).toBe(FileKind.DOCUMENT)
    expect(stored.createdById).toBe(leadId)
  })

  it('takes a DOCX as well as a PDF', async () => {
    const response = await publish(leadCookie, {
      name: 'handover.docx',
      bytes: docxBytes(),
    })
  ...
  it('refuses a file wearing an extension that is not its own', async () => {
    const response = await publish(leadCookie, {
      name: 'notes.pdf',
      bytes: new TextEncoder().encode('this is just some text'),
    })

    expect(response.status).toBe(400)
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining('PDF or DOCX'),
    })
  })

  it('refuses a plain zip renamed .docx', async () => {
    // A local file header and nothing Word would ever have written.
    const zip = new Uint8Array(40)
    zip.set([0x50, 0x4b, 0x03, 0x04], 0)
    new DataView(zip.buffer).setUint16(26, 8, true)
    zip.set(new TextEncoder().encode('logo.png'), 30)

    const response = await publish(leadCookie, { name: 'report.docx', bytes: zip })
  ...
  it('refuses a file over the cap with a 413 that names the limit', async () => {
    const oversized = new Uint8Array(
      env.MAX_DOCUMENT_FILE_MB * 1024 * 1024 + 128 * 1024,
    )
    oversized.set(new TextEncoder().encode('%PDF-1.7'), 0)

    const response = await publish(leadCookie, { bytes: oversized })

    expect(response.status).toBe(413)
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining(`${env.MAX_DOCUMENT_FILE_MB} MB`),
    })
  })

  it('refuses a member who does not run the project', async () => {
    await prisma.projectMember.create({
      data: { projectId, userId: paidId, rank: ProjectMemberRank.MEMBER },
    })

    const response = await publish(paidCookie)

    expect(response.status).toBe(403)
    expect(await prisma.projectDocument.count({ where: { projectId } })).toBe(0)
  })

  it('lets an officer publish, crediting themselves, without joining', async () => {
    const response = await publish(officerCookie, { authorUserId: officerId })

    expect(response.status).toBe(201)
    expect(await response.json()).toMatchObject({ authorName: 'PM Officer' })
  })

  it('refuses a credit naming somebody who is not on the project', async () => {
    const response = await publish(leadCookie, { authorUserId: paidId })

    expect(response.status).toBe(400)
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining('on this project'),
    })
  })

  it('credits a member of the project who is not the uploader', async () => {
    await prisma.projectMember.create({
      data: { projectId, userId: paidId, rank: ProjectMemberRank.MEMBER },
    })
    ...
    expect(response.status).toBe(201)
    expect(await response.json()).toMatchObject({ authorName: 'PM Paid' })
  })
  ...
  it('refuses the twenty-first, and says the number', async () => {
      ...
      Array.from({ length: 20 }, (_, index) =>
        prisma.projectDocument.create({
          data: {
            ...
            title: `Filler ${index}`,
            authorName: 'PM Lead',
            author: { connect: { id: leadId } },
            file: {
              create: {
                kind: FileKind.DOCUMENT,
                mimeType: 'application/pdf',
                byteSize: 8,
                originalName: `filler-${index}.pdf`,
                data: pdfBytes(),
                createdById: leadId,
              },
            },
    ...
    expect(response.status).toBe(409)
    expect(await response.json()).toMatchObject({
      error: expect.stringContaining('20 documents'),
    })
  })
  ...
  it('replaces the file, moves updatedAt, and leaves uploadedAt alone', async () => {
    ...
    expect(document['fileId']).not.toBe(first.fileId)
    expect(new Date(document['updatedAt']).getTime()).toBeGreaterThan(
    ...
    expect(
      await prisma.storedFile.count({ where: { id: document['fileId'] } }),
    ).toBe(1)
  })

  it('does not move updatedAt when only the title changes', async () => {
    const first = await published()
    vi.advanceTimersByTime(60_000)

    const response = await request(
      'PATCH',
      `/api/projects/${projectId}/documents/${first.id}`,
      leadCookie,
    ...
    expect(response.status).toBe(200)
    const document = (await response.json()) as Record<string, string>
    expect(document['title']).toBe('Design review (v2)')
    expect(document['updatedAt']).toBe(document['uploadedAt'])
  })

  it('refuses to touch a document belonging to another project', async () => {
    const first = await published()

    const response = await request(
      'PATCH',
      `/api/projects/${otherProjectId}/documents/${first.id}`,
      leadCookie,
      { title: 'Stolen' },
    )
  ...
  it('deletes the row and the bytes together', async () => {
    ...
    expect(response.status).toBe(200)
    expect(await prisma.projectDocument.count({ where: { id: first.id } })).toBe(0)
    expect(await prisma.storedFile.count({ where: { id: first.fileId } })).toBe(0)
  })

  it('serves them on the public project page, oldest first', async () => {
    await publish(leadCookie, { title: 'First' })
    await publish(leadCookie, { title: 'Second' })

    const response = await app.request(`/api/projects/${PREFIX}rover`)
    const project = (await response.json()) as {
      documents: { title: string; fileId: string; fileSize: number }[]
    }

    expect(project.documents.map((document) => document.title)).toEqual([
      'First',
      'Second',
    ])
    // Flattened, not nested: the browser wants an id to build a URL from.
    expect(project.documents[0]?.fileId).toEqual(expect.any(String))
    expect(project.documents[0]?.fileSize).toBeGreaterThan(0)
  })

  it('takes every document’s bytes with the project', async () => {
    const first = await published()
