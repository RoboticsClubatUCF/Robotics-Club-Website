...
describe('downscaleImage', () => {
  /**
   * jsdom cannot decode an image at all — `<img>` fires neither `onload` nor
   * `onerror` for a blob URL, so this is the hang the decode timeout exists
   * for, and it resolves rather than sitting there. That is the point twice
   * over: it is what a browser that cannot decode gets, and it is what makes
   * the whole upload path testable here without a canvas shim.
   *
   * Real timers, and a timeout past `DECODE_TIMEOUT_MS` — under fake timers
   * this would need advancing by hand, and testing.md is clear about mixing
   * those with async waits.
   */
