 ...
 *
 * `STORED_PREFIX` mirrors the constant in `server/src/files/files.ts`, where the same
 ...
 *
 * **`<a download>` does not work here and never will**: the attribute is
 * ignored on a cross-origin link, and the site and the API are always different
 * origins — that is the whole premise of this file. So "download" has to be
 * something the *server* decides, and `?download=1` is how the browser asks;
 * the file route answers it with `Content-Disposition: attachment`.
 *
 * Only documents read the flag. An image is not offered as a download anywhere,
 * and a print model is already forced to one.
 */
