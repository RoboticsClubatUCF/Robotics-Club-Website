...
import { hits, narrow, showAllLabel } from '../lib/catalogue'
import { FilterChips } from '../components/shared/FilterChips'
import { STATUS_TONE, shortDate } from '../lib/formats'
...
const quietButton = `${actionBase} btn-outline border-white/28 text-white hover:border-white hover:bg-white/6 hover:text-white`
      ...
      <>
        ...
        <FormPanel>
          <p className="text-dim text-sm leading-[1.7] text-pretty">
            The lending list and the borrowing queue are run by the officers
...
function LoanQueue() {
   ...
   * The server would fill the same gap on its own — see `routes/officer.ts` —
  ...
  return (
    <FormPanel>
      ...
      <p className="text-dim mb-4 text-[13px] leading-[1.6] text-pretty">
        <strong className="font-semibold text-white">Retiring</strong> takes
        something off the members' list and keeps every record of who borrowed
        it — that is the one you want for a tool that has broken, been sold or
        stopped being lent out.{' '}
        <strong className="font-semibold text-white">Deleting</strong> removes
        the item and its whole borrowing history from the database, for good.
        Use it for a row that should never have existed.
      </p>
      ...
      <form onSubmit={add} className="space-y-4">
        ...
        <div>
          ...
          {clash ? (
            <p role="status" className="text-warning mt-1 text-[12px] leading-[1.5]">
              ...
              them. Change that row's number above instead of adding a second
              one — or give this a name that tells the two apart.
            </p>
          ) : (
            <p className="text-faint mt-1 text-[12px] leading-[1.5]">
              One row per kind of thing, not per unit — how many there are is
              the next box.
            </p>
          )}
        </div>
        ...
        <div className="grid gap-3 wide:grid-cols-2">
          ...
          <div>
            ...
            />
            <p className="text-faint mt-1 text-[12px] leading-[1.5]">
              A week unless you change it. Members cannot ask for longer.
            </p>
          </div>
        </div>
        ...
        <div>
          ...
          />
          <p className="text-faint mt-1 text-[12px] leading-[1.5]">
            Shown under the name on the members' list, and searched.
          </p>
        </div>
      ...
      {doomed && (
        ...
        >
          ...
          <p>
            If the club still owns it and has only stopped lending it,{' '}
            <strong className="font-semibold text-white">retire it instead</strong> —
            that takes it off the members' list and keeps the history.
          </p>
        </ConfirmDialog>
      )}
    </FormPanel>
