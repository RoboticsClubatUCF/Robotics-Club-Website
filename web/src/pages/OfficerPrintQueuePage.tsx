...
import { hits } from '../lib/catalogue'
import { FilterChips } from '../components/shared/FilterChips'
import { STATUS_TONE, dateAndTime, fileSize } from '../lib/formats'
import type { StatusTone } from '../lib/formats'
import { storedFileUrl } from '../lib/storedFiles'
      ...
      <>
        ...
        <FormPanel>
          <p className="text-dim text-sm leading-[1.7] text-pretty">
            The print queue is run by the officers with access to the printers.
          </p>
        </FormPanel>
      </>
    )
...
function SettleDialog({
  ...
  return (
    ...
    >
      ...
      {cancelling && (
        <p>
          It is on a printer now — stopping it here does not stop the machine,
          so go and clear the bed as well.
        </p>
      )}

      <p>
        This deletes the uploaded model. {request.user.fullName}&apos;s record of
        the request stays — the file name, the settings and how it ended.
      </p>
        ...
        <>
          ...
          {over > 0 && (
            <p className="text-error mt-2 text-[12px] leading-[1.5]">
              That is {grams(over)} past what {request.user.fullName} has left.
              Marking it done will ask you to confirm.
            </p>
          )}
