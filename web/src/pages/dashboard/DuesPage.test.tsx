...
import type { ApiDuesStatus, ApiTerm } from '../../lib/api/api'
...
describe('DuesPage', () => {
    ...
    stubApi(
      duesStatus({
        membership: {
          ...
          // dues-bearing term to three weeks into the next made the split
  ...
  it('tells a paid-up member they are buying a term ahead', async () => {
    ...
    const note = await screen.findByText(/already paid through/i)
  ...
  describe('the free summer', () => {
    ...
    it('calls the opening weeks what they are, not a break', async () => {
      ...
      expect(await screen.findByText('THE FIRST THREE WEEKS')).toBeInTheDocument()
      ...
      expect(
        screen.getByText(/first three weeks of Fall 2026 are free/i),
      ).toBeInTheDocument()
    })
    ...
    it('still calls the gap before a term a break', async () => {
      ...
      expect(screen.queryByText('THE FIRST THREE WEEKS')).not.toBeInTheDocument()
    })
