...
import type { ApiMyProject, ApiProjectTeamView, ApiTask } from '../lib/api'
  ...
  if (!mine) {
    return (
      <>
        ...
        <FormPanel>
          <p className="text-dim text-sm leading-[1.7] text-pretty">
            Its public page has the full story — and the join button, if it is
            taking people.{' '}
            <Link
              to={`/projects/${slug}`}
                ...
                {mine ? (
                  <button
                    ...
                    className={`border-rule hover:border-primary mt-0.5 flex size-4 shrink-0 cursor-pointer items-center justify-center border text-[10px] leading-none transition-colors duration-200 disabled:opacity-50 ${
                      task.status === 'DONE' ? 'text-primary' : 'text-transparent'
                    }`}
                  >
                    ✓
                  </button>
                  ...
                  <span
                    ...
                    className={`border-rule mt-0.5 flex size-4 shrink-0 items-center justify-center border text-[10px] leading-none ${
                      task.status === 'DONE' ? 'text-faint' : 'text-transparent'
                    }`}
                  >
                    ✓
                  </span>
                )}
                <div className={`min-w-0 ${task.status === 'DONE' ? 'opacity-50' : ''}`}>
                  <p className={`text-sm leading-snug font-medium ${task.status === 'DONE' ? 'line-through' : ''}`}>
                    {task.title}
                  </p>
