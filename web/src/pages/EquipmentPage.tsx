...
import type { ApiEquipment, ApiLoan, LoanStatus } from '../lib/api'
import {
...
import type { WindowFault } from '../lib/borrowing'
import { narrow, showAllLabel } from '../lib/catalogue'
  ...
  return (
    <>
      ...
      <p className="text-dim mb-8 max-w-[42rem] text-sm leading-[1.7] text-pretty">
        Pick what you need, say when you need it and when you'll bring it back.
        An officer approves it before anything leaves the lab, and checks it
        back in when it returns. The bot messages you the day before it's due.
      </p>
...
function BorrowForm({
    ...
    return (
      <FormPanel>
        <p className="text-dim text-sm leading-[1.7] text-pretty">
          Nothing is on the lending list yet. Ask an officer — they can add it.
        </p>
      </FormPanel>
    )
  }
   ...
   * the server does. The judging is `lib/borrowing.ts`'s precisely so the two
        ...
        <p className="text-dim mb-4 text-sm leading-[1.7]">
          {query.trim()
            ...
            : shelf === 'MINE'
              ...
              : 'Everything is out at the moment. Ask again when something is back.'}
        </p>
      )}

      <fieldset className="grid gap-4 wide:grid-cols-2" disabled={sending}>
      ...
      <p role="status" className="mt-3 min-h-5 text-[13px] leading-[1.5]">
        ...
        {state.status === 'sent' && (
          <span className="text-success">
            Asked. An officer has been told — watch for it below.
          </span>
        )}
      </p>
    </form>
...
function BorrowWindow({
  ...
  return (
    <FormPanel>
      ...
      <fieldset className="mb-4 grid gap-3 wide:grid-cols-2" disabled={disabled}>
