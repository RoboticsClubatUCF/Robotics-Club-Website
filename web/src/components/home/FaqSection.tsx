...
/**
 * The FAQ, as a stack of native `<details>` disclosures, with the contact form beside it.
 *
 * The two share a row on purpose. The FAQ answers what is already written down and the form covers
 * what is not, so anyone who reads to the bottom without finding their question has somewhere to
 * put it — and the answers are set to a reading measure, which left half the section empty at
 * width.
 *
 * `<details>` rather than a hand-rolled accordion because the browser already supplies the open
 * state, the keyboard handling and the role — and because the closed content stays findable by the
 * browser's own in-page search, which a component that unmounts its answers would not be. DaisyUI's
 * `collapse` classes are built to sit on exactly this markup.
 *
 * The answers were copy and are data now. Dues, lab hours and staff change more often than the site
 * deploys — two of the eight name a price and one names a person — so officers write them at
 * `/dashboard/officer/front-page`, and `HomePage` fetches them with the rest of the page's words.
 *
 * The contact form is drawn in every state, including the one where the questions could not be
 * loaded. It is the half of this section that does not need the API to be useful, and a visitor
 * whose question is not answered is exactly who it is for.
 */
export function FaqSection({ copy }: { copy: ApiState<ApiFrontPage> }) {
  ...
  return (
    ...
    >
      {/* One column below the breakpoint, with the form under the answers. The form's track is
          fixed: a text input that grows to a thousand pixels on a wide monitor looks like a
          mistake, and the disclosure rows are happy to take whatever is left — a full-width rule
          with the question at one end and the marker at the other is the same row the schedule and
          the stat strip use. */}
        ...
        <div>
          ...
          {faqs.map((faq) => (
            ...
            >
              ...
              {/* `group-open:` is what makes this a one-shot on open rather than a permanent class:
                  the animation only exists while the details is open, so closing and reopening
                  replays it. Height is left alone — `auto` is not interpolable, and the fade plus
                  the short slide is enough to stop the answer appearing from nowhere. */}
