 ...
 *
 * The requirement comes before the field on purpose. Membership is for current UCF students
 * and the address has to be one they can open right now — somebody who reads that after
 * typing a Gmail address has already wasted the effort, and somebody who reads it after the
 * email is sent has wasted two minutes waiting for mail they can't reach.
 *
 * `POST /api/signup/start` stores nothing but the address, so this step creates no account
 * and can be abandoned without leaving anyone to clean up after.
 */
 ...
 *
 * Starting again is one click from asking for a second email, and the usual reason somebody
 * reaches for it is that the first hasn't arrived in the four seconds they've been watching
 * for it. Mail takes longer, every extra request pushes the previous link out of date, and
 * enough of them in a row is how a sender's reputation gets spent.
 */
...
/**
 * Three of these are worth telling apart by status alone, the same three the contact form
 * separates: an unreachable API, a rate limit, and a rejected field. The rest are sentences
 * only the server can write, so they come back on the error and are shown as sent.
 */
...
export function SignupStart() {
   ...
   *
   * The expected thing to do on this screen is switch to a mail app, and a background tab
   * has its timers throttled to roughly one a minute — a countdown built from ticks would
   * still be showing twenty-odd seconds after five minutes away.
   */
  useEffect(() => {
    if (!restartAt) return
  ...
  const send = (event: FormEvent<HTMLFormElement>) => {
    ...
    postJson<ApiSignupStarted>('/signup/start', {
      email,
      // The server asks for this too. The checkbox is a promise the browser makes, and the
      // requirement is the entire point of this step.
      acknowledged: true,
    })
      .then((sent) => {
        setState({
          status: 'sent',
          // The server's spelling, lowercased and trimmed — which is what it actually
          // mailed, and worth showing rather than what was typed.
          email: sent.email,
  ...
  if (state.status === 'sent') {
    return (
      <>
        ...
        {/* The single most common reason someone never finishes signing up, so it gets the
            accent panel rather than small print under the fold. University filters are
            aggressive about mail from addresses nobody in the domain has written to before. */}
        ...
        {/* Said plainly because the alternative is somebody sitting on this tab waiting for
            it to advance by itself. It never does: the rest of signup hangs off the token in
            the link, which is what lets them finish on their phone after starting on a
            laptop. */}
        ...
        >
          {/* Only the second half is the link. The first half is the question it answers,
              and painting both gold made the whole line read as one long button. */}
  ...
  return (
    <>
      ...
      {/* Directly under the requirement, because this is the line somebody reads the moment
          they find out it rules them out — a page that says "students only" and stops is a
          dead end for every school team, mentor and volunteer who came here wanting to
          build something.

          A plain `<a>`, not a `<Link>`: the target is a section of the front page, and a
          router navigation would take the hash over and then not scroll to it. Written
          `/#partners` rather than `#partners` for the reason the nav's links are. */}
      ...
      <form onSubmit={send} className="mt-7 flex flex-col gap-5">
        <div>
          ...
          <input
            ...
            /* The browser's own check for the shape of an address, and a pattern for the
               part that's this club's rule rather than the format's. The server enforces
               both again; neither is a boundary.

               Spelled out a character at a time because `pattern` takes no flags and is
               case-sensitive: the server lowercases before it compares, so a plain
               `@ucf\.edu` would block someone typing `@UCF.EDU`. */
        ...
        {/* Live, because the outcome arrives long after the click and the button label is
            the only other thing that moves. Always rendered so the region exists before it
            has anything to say. */}
